...
if __name__ == "__main__":
    ...
    for _ in range(iterations):
        ...
        mine_ssim_val_same = fused_ssim3d_by_2d(img1_mine_same, img2_mine_same).double()
        mine_ssim_val_valid = fused_ssim3d_by_2d(img1_mine_valid, img2_mine_valid, "valid").double()
    ...
    begin = time.time()
    for _ in range(iterations):
        mine_ssim_val = fused_ssim3d_by_2d(img1, img2)
    fused_ssim_module.synchronize()  # Ensure GPU operations complete before timing
    end = time.time()
    ...
    begin = time.time()
    for _ in range(iterations):
        mine_ssim_val = fused_ssim3d_by_2d(img1, img2, train=False)
    fused_ssim_module.synchronize()  # Ensure GPU operations complete before timing
    end = time.time()
