import torch
from fused_ssim3d import fused_ssim3d_by_2d
...
if __name__ == "__main__":
    ...
    for d in dimensions:
        ...
        begin = time.time()
        for _ in range(iterations):
            mine_ssim_val_same = fused_ssim3d_by_2d(img1_mine_same, img2_mine_same)
    ...
    plt.title(f"3D_by_2D Training Benchmark on {gpu}.")
    plt.savefig(os.path.join("..", "images", f"3D_by_2D_training_time-{gpu.lower().replace(' ', '-')}.png"), dpi=300)
    ...
    for d in dimensions:
        with torch.no_grad():
            ...
            begin = time.time()
            for _ in range(iterations):
                mine_ssim_val_same = fused_ssim3d_by_2d(img1_mine_same, img2_mine_same, train=False)
            fused_ssim_module.synchronize()  # Ensure GPU operations complete before timing
            end = time.time()
            data["fused-ssim"].append((end - begin) / iterations * 1000)
    ...
    plt.title(f"3D_by_2D inference Benchmark on {gpu}.")
    plt.savefig(os.path.join("..", "images", f"3D_by_2D_inference_time-{gpu.lower().replace(' ', '-')}.png"), dpi=300)